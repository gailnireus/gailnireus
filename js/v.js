...
function showV(){
  ...
  for(let y=0; y<gridCount; y++){
    for(let x=0; x<gridCount; x++){
      ...
      // population update awal
      if(populations[coord]){
        const p = populations[coord];
        cell.dataset.tooltip += `\nType: ${p.type}\nPopulation: ${p.value}`;
        const density = p.value / popTypes[p.type].max;
        if(density < 0.33) cell.style.background = '#444';
        else if(density < 0.66) cell.style.background = '#272';
        else cell.style.background = '#722';
      }
...
function updatePopulation(){
  for(const coord in populations){
    ...
    // update cell color ikut density relative max
    const cell = document.querySelector(`[data-coord="${coord}"]`);
    if(cell){
      const density = pop / rule.max;
      if(density < 0.33) cell.style.background = '#444';
      else if(density < 0.66) cell.style.background = '#272';
      else cell.style.background = '#722';
    }
